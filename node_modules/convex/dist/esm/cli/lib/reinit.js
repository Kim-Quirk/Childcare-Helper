...
export async function reinit(ctx, config, saveUrl = "ask", promptForAdditionalSteps, options = { allowExistingConfig: false }) {
...
`);
  ...
  {
    const { projectConfig: projectConfig2 } = await pullConfig(
      ctx,
      projectSlug,
      teamSlug,
      url,
    ...
    await writeProjectConfig(ctx, projectConfig2);
  }
