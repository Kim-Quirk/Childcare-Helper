...
async function main() {
  ...
  program.name("convex").usage("<command> [options]").addCommand(login, { hidden: true }).addCommand(init, { hidden: true }).addCommand(reinit, { hidden: true }).addCommand(dev).addCommand(deploy).addCommand(deployments, { hidden: true }).addCommand(typecheck, { hidden: true }).addCommand(auth).addCommand(convexImport).addCommand(codegen).addCommand(dashboard).addCommand(docs).addCommand(update).addHelpCommand("help <command>", "Show help for given <command>").version(version).configureHelp({ visibleOptions: () => [] }).showHelpAfterError();
  try {
