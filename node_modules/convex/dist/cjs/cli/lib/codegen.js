...
function writeFile(ctx, filename, source, dir, dryRun, debug, quiet, filetype = "typescript") {
  ...
  if (debug) {
    (0, import_context.logMessage)(ctx, `# ${filename}`);
    (0, import_context.logMessage)(ctx, formattedSource);
    return;
  }
  if (dryRun) {
    if (ctx.fs.exists(dest)) {
      ...
      if (fileText !== formattedSource) {
        (0, import_context.logMessage)(ctx, `Command would replace file: ${dest}`);
      }
    } else {
      (0, import_context.logMessage)(ctx, `Command would create file: ${dest}`);
    }
    return;
  }
  if (!quiet) {
...
async function doApiCodegen(ctx, functionsDir2, codegenDir, dryRun, debug, quiet = false, commonjs = false) {
  const modulePaths = (await (0, import_bundler.entryPoints)(ctx.fs, functionsDir2, false)).map(
...
async function doCodegen({
  ...
  await (0, import_fs.mkdtemp)("_generated", async (tempCodegenDir) => {
    ...
    await (0, import_typecheck.typeCheckFunctionsInMode)(ctx, typeCheckMode, funcDir);
  });
}
