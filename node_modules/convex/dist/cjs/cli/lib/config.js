...
async function configFromProjectConfig(ctx, projectConfig, configPath, verbose) {
  ...
  try {
    ...
    (0, import_context.pauseSpinner)(ctx);
    modules = await (0, import_bundler.bundle)(
      ctx.fs,
      baseDir,
      entryPoints.isolate,
      true,
    ...
    (0, import_context.resumeSpinner)(ctx);
    if (verbose) {
    ...
    (0, import_context.logFailure)(ctx, "Error: Unable to bundle Convex modules");
    if (err instanceof Error) {
      console.error(import_chalk.default.gray(err.message));
    }
    return await ctx.crash(1, "invalid filesystem data", err);
  ...
  try {
    const res = await client.post(
      ...
      { version: import__.version, adminKey },
      {
        ...
        headers: {
          "Convex-Client": `npm-cli-${import__.version}`
        }
      }
    );
...
function configJSON(config, adminKey, schemaId) {
  const projectConfig = {
...
async function pushConfig(ctx, config, adminKey, url, schemaId) {
  const serializedConfig = configJSON(config, adminKey, schemaId);
  try {
    await import_axios.default.post(`${url}/api/push_config`, serializedConfig, {
      maxContentLength: Infinity,
