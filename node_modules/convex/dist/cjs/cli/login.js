...
).addOption(new import_commander.Option("--override-auth-url <url>").hideHelp()).addOption(new import_commander.Option("--override-auth-client <id>").hideHelp()).addOption(new import_commander.Option("--override-auth-username <username>").hideHelp()).addOption(new import_commander.Option("--override-auth-password <password>").hideHelp()).addOption(new import_commander.Option("--no-opt-in").hideHelp()).action(async (options, cmd) => {
  const ctx = import_context.oneoffContext;
  if (await (0, import_login.checkAuthorization)(ctx) && !options.force) {
    console.log(
