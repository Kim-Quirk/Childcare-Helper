...
).addOption(new import_commander.Option("--url <url>").hideHelp()).addOption(new import_commander.Option("--admin-key <adminKey>").hideHelp()).argument("<tableName>", "Destination table name").argument("<path>", "Path to the input file").showHelpAfterError().action(async (tableName, path, options) => {
  const ctx = import_context.oneoffContext;
  let format = options.format;
  const pathParts = path.split(".");
  if (pathParts.length > 1) {
    const fileType = pathParts[pathParts.length - 1];
    const formatToFileType = {
      csv: "csv",
      jsonLines: "jsonl",
      jsonArray: "json"
    };
